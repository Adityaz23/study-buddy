...
const ExploreUserCard = ({
  ...
  return (
    ...
    >
      ...
      />
      {item.online && (
        <View className="w-3 h-3 rounded-full bg-accent-secondary absolute left-[50px] top-[48px] border-2 border-surface" />
      )}
      ...
      {creating === item.id ? (
        ...
        <View className="w-9 h-9 rounded-xl bg-primary/20 justify-center items-center">
          <MaterialIcons name="chat-bubble" size={16} color={COLORS.primary} />
        </View>
      )}
    </Pressable>
  );
